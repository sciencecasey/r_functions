...
sepal_Length <- data.frame(
  testy(iris$Sepal.Length[1:50], 1),
...
sepal_Length <-
  cbind(sepal_Length, Feature = rep("Sepal Length", 7))


# repeat with Sepal Width
sepal_width <- data.frame(
  testy(iris$Sepal.Width[1:50], 1),
...
petal_Length <- data.frame(
  testy(iris$Petal.Length[1:50], 1),
...
petal_Length <-
  cbind(petal_Length, Feature = rep("Petal Length", 7))

# repeat with petal width
petal_width <- data.frame(
  testy(iris$Petal.Width[1:50], 1),
...
# combine features
output <-
  rbind(sepal_Length, sepal_width, petal_Length, petal_width)
