...
# expected value using monte carlo methods
expected_value_monte_carlo <- function(f, from, to, ...){
    # draw from the uniform distributions from, to
    u <- runif(10000, 0, (to-from))
    # add the multiplier to f
    fm <- (to-from)*f(u, ...)
    return(mean(na.omit(fm)))
}

expected_value_monte_carlo(f=function(x)sin(x), 0, 1)
