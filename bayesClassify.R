...
#out <- bayes_classifier.separated(test = testy, training = trainy, testClassList = listy, trainClassList = listy)
#which(out$assigned != c(rep(1, 10), rep(2, 10), rep(3, 10))) # all accurately classified!
# bayes_classifier.accuracy(out)


# Another example how to use
# make dfs with different classifications (species) using the function above
# s <- extract_training((iris %>% filter(Species == 'setosa')), train_amount = .5)
# vi <- extract_training((iris %>% filter(Species == 'virginica')), train_amount = .5)
# ve <- extract_training((iris %>% filter(Species == 'versicolor')), train_amount = .5)
# separate test and train
# testy <- rbind(s$test_dat,
               # vi$test_dat, 
               # ve$test_dat)
# trainy <- rbind(s$training_dat, 
                # vi$training_dat, 
                # ve$training_dat)
# rm(s, vi, ve)
# make classification lists
#listy_test <- list(which(testy$Species=='setosa'), 
                   # which(testy$Species=='virginica'), 
                   # which(testy$Species=='versicolor'))
# listy_train <- list(which(trainy$Species=='setosa'), 
                    # which(trainy$Species=='virginica'), 
                    # which(trainy$Species=='versicolor'))
# out <- bayes_classifier.separated(test = testy[, -5], 
                                  # training = trainy[, -5], 
                                  # testClassList = listy_test, 
                                  # trainClassList = listy_train)
# bayes_classifier.accuracy(out) # 100 % training accuracy!

read.csv('~/OneDrive/JHU/Data Visualization/assignments/visit_1_data.csv') %>%
    select(unique('SWANID')) %>%
    nrow()
